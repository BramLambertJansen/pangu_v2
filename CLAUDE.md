...
- [x] **Kleurconsolidatie** — kanaal-tokens (`--violet-rgb`/`--gold-rgb`/`--teal-rgb`/`--crimson-rgb`/`--azure-rgb`/`--void-rgb`/`--muted-rgb`/`--surface-rgb`/`--ink-rgb`); gekleurde accent- en oppervlakte-tints gebruiken `rgb(var(--x-rgb) / α)`; accent-as overschrijft `--violet-rgb`. **Bewuste uitzonderingen** (blijven hardcoded, niet thema-gedreven): foto-overlays en tekst-/box-shadow-scrims over afbeeldingen (`CharacterCard`, `CampaignCard`, `WorldCard`, hero-scrims op detailpagina's, modal-image-scrims), en literale muntkleuren (`#e5e7eb` platina, `#c0a060` elektrum, `#b87333` koper in `CharacterStatsTab`/`CharacterInventoryTab`)
...
- [x] **Primitive-hergebruik in de app** — `Tabs` (nieuw `variant='pill'|'line'` + `ReactNode`-labels) vervangt hand-gerolde tab-bars in `CharacterDetailPage`, `CampaignDetailPage` (line), `CampaignItemsPage`, `SettingsPage`; `SearchBar` in `CharactersPage`; `StatusBadge` (nieuw `tone='solid'|'soft'`) in `CharacterDetailPage` (soft) + `CampaignDetailPage` (solid, via `campaignStatusColor`); `Chip` (nieuw `tone='neutral'|'violet'|'crimson'|'teal'`) voor de gekleurde pills in `EncounterRunPage`; `CombatTracker` ingebed als read-only initiatiefvolgorde-preview op `EncounterDetailPage`
- [x] **Standaardisatie fase 0 (fundering & vangrails)** — `@theme inline` uitgebreid met radius-brug (`--radius-* = var(--r-*)` → `rounded-*` volgt tokens); opaciteitsschaal (`--o-04…--o-50`) + composiet-transities (`--tr-fast`/`--tr-base`); `scripts/check-inline-styles.mjs` + `npm run check:styles` vangrail (warn → `--strict` na sweep); `npm run type-check` nu `tsc -b` (was no-op op leeg root-project); twee toegestane styling-vocabulaires gedocumenteerd in `docs/design-system/03`
- [x] **Standaardisatie fase 1 (primitive-laag)** — nieuwe primitives `FormField`, `Select`, `Textarea`, `IconButton`, `NumberStepper`, `SegmentedControl` (radiogroup + arrow-keys); `Input` rendert nu canonieke `.input`/`.input-error` via `FormField`; `Badge` → `.badge-*`, `StatusBadge` → `--sb-color` CSS-var (color-mix uit TSX), `Avatar` → `.avatar*`, `Spinner` → `.spinner*`, `KbdHint` → `.kbd`, `ForgeCard`-interieur → `.forge-card-*`; alles zichtbaar in `/design-system`; tests in `src/test/ui-primitives.test.tsx`
- [ ] **Fase 2** — DS-v2 als nieuwe basis (alleen token-waarden: rondere radii etc.)
- [ ] **Fase 3 (sweep)** — edit-pagina's → `FormField`/`Select`/`Textarea`; inline-styles + `.pangu-*` retireren (zie `scripts/check-inline-styles.mjs` baseline)
