...
export default function LoreEditPage() {
  ...
  async function handleGenerateDescription() {
    ...
    const categoryHint = form.lore_category ? ` (categorie: ${form.lore_category})` : ''
      ...
      const reply = await ask([
        {
          ...
          content:
            ...
            `Schrijf een rijke, meeslepende lore-beschrijving (3–5 zinnen in het Nederlands) over ` +
            `"${name}"${categoryHint}${subtitleHint}. ` +
            `Beschrijf oorsprong, betekenis en hoe het de wereld beïnvloedt; geef het mysterie en diepgang.` +
            existingHint +
    ...
    <div style={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
      <div style={{ maxWidth: 820, width: '100%' }}>
        <Breadcrumbs
          showBack
          onBack={handleBack}
          items={[{ label: loreData.name, to: `/lore/${id}` }]}
          current="Bewerken"
        />
