...
import { useCharacterSpells, useAddCharacterSpell, useRemoveCharacterSpell, useToggleSpellPrepared } from '@/hooks/queries/useCharacterSpells'
...
export default function CharacterDetailPage() {
  ...
  const { data: characterSpells, isLoading: isLoadingCharacterSpells } = useCharacterSpells(id)
  const { data: spellLibrary } = useSpells()
  const addCharacterSpell = useAddCharacterSpell(id!)
  const removeCharacterSpell = useRemoveCharacterSpell(id!)
  const toggleSpellPrepared = useToggleSpellPrepared(id!)
        ...
        {!spellAbility ? (
          ...
          <div>
            ...
            <div className="pangu-surface" style={{ padding: 24, marginTop: 16 }}>
              ...
              {/* Spell picker */}
              {spellPickerOpen && (
                <div id="spell-picker" style={{ marginBottom: 16, padding: 16, background: 'var(--surface)', borderRadius: 8, border: '1px solid var(--hairline)' }}>
                  ...
                  {(spellLibrary ?? []).length === 0 ? (
                    ...
                    <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 4 }} role="list">
                      {(spellLibrary ?? []).map(spell => {
                        const alreadyAdded = (characterSpells ?? []).some(cs => cs.spell_id === spell.id)
                        return (
                          <li key={spell.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                            <span style={{ fontSize: 13, color: alreadyAdded ? 'var(--muted)' : 'var(--ink)' }}>
                              <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: spellSchoolColor[spell.school], marginRight: 6 }}>
                                {spell.level === 0 ? 'Kantrip' : `Niv ${spell.level}`}
                              </span>
                              {spell.name}
                            </span>
                            {alreadyAdded ? (
                              <span style={{ fontSize: 10, color: 'var(--muted)', fontStyle: 'italic' }}>Toegevoegd</span>
                            ) : (
                              <button
                                type="button"
              ...
              {isLoadingCharacterSpells ? (
                ...
                <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 8 }} role="list" aria-label="Gekende spreuken">
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'var(--surface)' }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <p style={{ margin: '0 0 1px', fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: spellSchoolColor[spell.school] }}>
                              {spell.level === 0 ? 'Kantrip' : `Niveau ${spell.level}`} · {spellSchoolLabel[spell.school]}
                              {spell.concentration && <span style={{ marginLeft: 6, color: 'var(--gold)' }}>C</span>}
                              {spell.ritual && <span style={{ marginLeft: 4, color: 'var(--teal)' }}>R</span>}
                            </p>
